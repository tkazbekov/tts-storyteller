...
async def list_pools_endpoint() -> list[str]:
    """List all available voice pools."""
    pool_repo = get_pool_repository()
    return sorted(await pool_repo.list_pools())


@router.get("/voices/pools/{poolName}")
async def get_pool_endpoint(poolName: str) -> list[Voice]:
    """Get all voices in a pool."""
    pool_repo = get_pool_repository()
    voice_ids = await pool_repo.get_voices(poolName)
    if not voice_ids:
...
async def create_pool_endpoint(poolName: str, pool: VoicePool) -> VoicePool:
    """Create a new pool."""
    if pool.name != poolName:
        raise HTTPException(status_code=400, detail="poolName in path must match name in body")

    pool_repo = get_pool_repository()
    all_pools = await pool_repo.get_all_pools()
    ...
    await pool_repo.save_pool(poolName, pool.voiceIds)

    return pool


@router.put("/voices/pools/{poolName}")
async def update_pool_endpoint(poolName: str, pool: VoicePool) -> VoicePool:
...
async def delete_pool_endpoint(poolName: str) -> None:
    """Delete a pool."""
    pool_repo = get_pool_repository()
    all_pools = await pool_repo.get_all_pools()
    if poolName not in all_pools:
    ...
    await pool_repo.delete_pool(poolName)
